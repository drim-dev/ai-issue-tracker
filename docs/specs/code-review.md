...
| `submit_review` | Терминальный tool: агент вызывает его ровно один раз с финальным `{ summary, verdict, comments[] }`; `input` биндится напрямую в `ReviewResult`. |
...
- **Агентный цикл** — Claude Messages API, `MaxTurns = 20` (override через `--max-turns`); цикл крутится на `stop_reason: "tool_use"` и завершается вызовом tool'а `submit_review`. `end_turn` без `submit_review`, любой другой `stop_reason` или превышение `MaxTurns` → `502 code_review:agent:failed`.
- **Prompt caching** — `cache_control: ephemeral` на последнем блоке system-промпта и на терминальном tool'е `submit_review` (всегда последний элемент массива `tools`). Initial user message не кешируется.
- **Финальный ответ** — вызов tool'а `submit_review` с `input` по схеме `{ summary, verdict, comments[] }`. Аргументы tool-call'а биндятся структурно (без парсинга текста/JSON). Неизвестный `verdict`/`category` → `502 code_review:agent:failed`.
...
- **Sinks** — `ConsoleReviewSink` рендерит markdown (`## Summary` → `## Inline Comments` с группировкой по файлу → `## Verdict`) и вызывается всегда; `GitHubReviewSink` публикует через `POST /repos/{o}/{r}/pulls/{n}/reviews` с `event` по вердикту, вызывается только при `PostToGitHub = true`. Если токен принадлежит автору PR (`GET /user` совпадает с `PrMetadata.Author`), `event` `APPROVE`/`REQUEST_CHANGES` принудительно понижается до `COMMENT` — GitHub иначе отвергает запрос с `422`. Sinks дёргаются вызывающим (CLI), handler чистый.
